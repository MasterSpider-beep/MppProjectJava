...
public class DBTicketRepository implements TicketRepository{
    ...
    private static Ticket getTicket(int id, ResultSet result) throws SQLException {
        ...
        List<String> tourists = Arrays.stream(touristsTemp.split(";")).filter(tourist->{return !tourist.isBlank();}).toList();
    ...
    public boolean addOne(Ticket ticket) {
        ...
        Connection connection = dbUtils.getConnection();
        try(PreparedStatement statement = connection.prepareStatement("INSERT INTO Tickets(clientName, tourists, address, noSeats, flight_id) VALUES (?,?,?,?, ?)")){
            statement.setString(1,ticket.getClientName());
            String tourists;
            List<String> touristsTemp = ticket.getTourists();
            StringBuilder builder = new StringBuilder();
            for (String tourist:touristsTemp){
                builder.append(tourist).append(";");
            }
            ...
            statement.setInt(5,ticket.getFlightId());
